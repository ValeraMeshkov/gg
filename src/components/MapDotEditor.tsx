...
export function MapDotEditor({ mapId, onMapIdChange }: MapDotEditorProps) {
  ...
  const isSpotMoved = useCallback(
    (spot: number) => {
      ...
      return (
        Math.abs(pos.x - orig.x) > 0.05 ||
        Math.abs(pos.y - orig.y) > 0.05
      );
    },
  ...
  return (
    <div className={styles.root}>
      <div className={styles.toolbar}>
        ...
        <button type="button" className={styles.btnPrimary} onClick={persistLayout}>
          Сохранить
        </button>
        {import.meta.env.DEV ? (
          <button type="button" className={styles.btn} onClick={() => void writeToCode()}>
            Записать в код
          </button>
        ) : null}
        <button type="button" className={styles.btnDanger} onClick={toggleHidden}>
      ...
      <div className={styles.layout}>
        ...
        >
          ...
          {territoryMap.territories.map((territory, index) => {
            ...
            const pos = positions[spot] ?? { x: territory.dotX, y: territory.dotY };
            return (
              <g
                key={territory.id}
            ...
            <ul className={styles.spotList}>
              {territoryMap.territories.map((territory, index) => {
                ...
                return (
                  <li
                    key={spot}
                    className={`${styles.spotListItem}${selected ? ` ${styles.spotListItemSelected}` : ""}${hidden ? ` ${styles.spotListItemHidden}` : ""}`}
                  >
                    <button
                      type="button"
